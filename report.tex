...
This study addressed the critical challenge of "scanner blindness" in modern Single Page Applications, where traditional crawling and stateless fuzzing fail to uncover deep business logic vulnerabilities. By implementing a Clean-Slate, template-driven framework, we demonstrated that combining automated endpoint discovery with state-aware execution significantly improves detection coverage for complex flaws such as IDOR and logic bypasses.

The experimental results against OWASP Juice Shop validated that \textbf{template quality and authentication context are the primary determinants of detection capability}—not the scanning engine itself. When Nuclei was equipped with custom templates and Bearer token authentication, it successfully detected critical vulnerabilities including path traversal, mass assignment, JWT 'none' algorithm, negative quantities, and price manipulation.

However, Squeezer demonstrated significant practical advantages: **3× more unique findings** (18 vs 6) and **4.33× more total matches** (39 vs 9) with identical template count (14), achieved through endpoint expansion macros and field-name auto-discovery. The framework's clean-slate lab mode ensures reproducible tests by provisioning ephemeral targets, eliminating state pollution across runs.

The benchmark also revealed an important trade-off: Nuclei is **5.4× faster** (0.24s vs 1.29s), making it better suited for rapid assessments and CI/CD integration, while Squeezer's deeper inspection logic provides superior coverage for comprehensive security audits. This suggests that the optimal tool depends on the specific use case—speed vs. coverage—rather than one tool being universally superior.

Ultimately, this work suggests that the future of DAST lies not in faster generic fuzzing, but in smarter template design that leverages endpoint discovery, stateful execution, and automated expansion to maximize coverage with minimal manual maintenance.
